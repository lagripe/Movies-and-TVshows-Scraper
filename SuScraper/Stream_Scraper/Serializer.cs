...
namespace Stream_Scraper
{
	...
	public class Serializer
    {
        ...
        private Dictionary<Dictionary<string, string>, bool> seenHashtables; //for serialize (to infinte prevent loops)
        private Dictionary<List<Dictionary<string, string>>, bool> seenArrayLists; //for serialize (to infinte prevent loops) lol
        ...
        {
            this.seenArrayLists = new Dictionary<List<Dictionary<string, string>>, bool>();
            this.seenHashtables = new Dictionary<Dictionary<string, string>, bool>();
        ...
        private StringBuilder serialize(object obj, StringBuilder sb)
        {
            ...
            {
                if (this.seenArrayLists.ContainsKey((List<Dictionary<string, string>>)obj))
                    return sb.Append("N;");//cycle detected
                else
                    this.seenArrayLists.Add((List<Dictionary<string, string>>)obj, true);

                List<Dictionary<string, string>> a = (List<Dictionary<string, string>>)obj;
                sb.Append("a:" + a.Count + ":{");
                for (int i = 0; i < a.Count; i++)
                {
                    this.serialize(i, sb);
                    this.serialize(a.ElementAt(i), sb);
                }
                sb.Append("}");
                return sb;
            }
            else if (obj is IDictionary)
            {
                ...
                if (this.seenHashtables.ContainsKey((Dictionary<string,string>)obj))
                    return sb.Append("N;");//cycle detected
                else
                    this.seenHashtables.Add((Dictionary<string, string>)obj, true);

                Dictionary<string, string> a = (Dictionary<string, string>)obj;
                sb.Append("a:" + a.Count + ":{");
